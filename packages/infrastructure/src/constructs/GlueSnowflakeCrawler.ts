...
import {CfnConnection, CfnCrawler} from "aws-cdk-lib/aws-glue";
...
export class GlueSnowflakeCrawler extends Construct {
	...
	constructor(scope: Construct, id: string, config: GlueSnowflakeCrawlerConfig) {
		...
		const driverBucket = new Bucket(this, "SnowflakeDriverBucket", {
		...
		const asset = new BucketDeployment(this, "SnowflakeJdbcDriverDeployment", {
			destinationBucket: driverBucket,
		...
		driverBucket.grantReadWrite(glueCrawlerRole)
